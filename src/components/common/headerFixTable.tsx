...
import { TableRowType, TableType } from "@/types/common";
...
const TableRow = ({ column, data, onClick }: TableRowType) => {
  ...
  return (
    <td
      className={`flex-1 py-[20px] text-[14px] whitespace-normal`}
      onClick={onClick}
    >
      <div>
        <span>
        ...
        <table className={"w-full"}>
          <thead className="w-full">
            <tr className={"w-full flex"}>
              ...
              {isSelectedCheckbox ? <th className="min-w-[40px]"></th> : null}
              {columns.map((column, idx) => (
                <th
                  key={`column${idx}`}
                  className={cls("flex-1 h-[60px] text-[14px]")}
                >
          ...
          <tbody className="text-center divide-y-[1px] max-h-[60vh] block overflow-y-scroll">
            {data.map((item: any, idx: any) => {
              // const isRowExpanded = expandedRows.includes(row.values.id);
              return (
                <tr
                  key={`data${idx}`}
                  className="w-full flex cursor-pointer hover:bg-[#eefdd3] transition-colors"
                >
                  {isCheckboxUse ? (
                  ...
                  {isSelectedCheckbox && (
                    <td
                      className="min-w-[40px] py-[20px] text-[14px] whitespace-normal"
